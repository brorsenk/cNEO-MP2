...
def t_amps_en_only(self, int j_idx, lagr_multipliers, double[:,:,:,:] t_electronic_nuclear, double[:,:,:,:] l_electronic_nuclear, two_int_en_t, double[:,:] ncF_eMO, double[:,:] ncF_nMO_j, double[:,:,:] integrals_r_j):
    '''
    ...
    cdef double coulomb_term_en = 0
    cdef double sum_over_c_not_a_en = 0
    cdef double sum_over_C_not_A_en = 0
    cdef double sum_over_k_not_i_en = 0
    cdef double sum_over_K_not_I_en = 0
    ...
    cdef int p_nocc = int(self.num_ovt[j_idx][0,0])
    cdef int p_nvir = int(self.num_ovt[j_idx][0,1])
    cdef int p_tot = int(self.num_ovt[j_idx][0,2])
    ...
    cdef int i, a, I, A, c, k, C, K, P, Q, R, T, x

    # Initialization and building of the orbital energy difference tensor.
    cdef double[:,:,:,:] e_en = numpy.zeros((e_nocc, e_nvir, p_nocc, p_nvir))
    ...
    cdef double[:,:,:,:] T_old_en = t_electronic_nuclear
    cdef double[:,:,:,:] T_new_en = numpy.zeros((e_nocc, e_nvir, p_nocc, p_nvir))
    cdef double[:,:,:,:] L_old_en = l_electronic_nuclear
...
#    TPIMO_en = eri_ep.reshape(e_nocc, e_nvir, p_nocc, p_nvir)
    ...
    cdef double[:] virtual_gradient_term_en = numpy.zeros((3,))
    cdef double[:] occupied_gradient_term_en = numpy.zeros((3,))
