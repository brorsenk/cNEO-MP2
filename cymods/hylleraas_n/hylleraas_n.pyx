...
def Hylleraas_energy_n(self, t_amps_n, l_amps_n, two_int_n_t, two_int_n_l, fock_n):
    ...
    cdef double gt_sum_n_total = 0
    cdef double gl_sum_n_total = 0
    cdef double Ci_sum_n_total = 0
    cdef double Cj_sum_n_total = 0
    cdef double Ki_sum_n_total = 0
    cdef double Kj_sum_n_total = 0
    cdef double iterative_mp2_nuclear_new = 0
    ...
    cdef int i, j, I, A, J, B, Ci, Cj, Ki, Kj
    cdef double zero = 0
    ...
    for i in range(nuclei):
        ...
        for j in range(nuclei):
            ...
            if (i == j):
                ...
                print(tracemalloc.get_traced_memory(), 'HI', flush=True)
                #-----------------------------------------------------------------
    ...
    print(tracemalloc.get_traced_memory(), 'Bye', flush=True)
    cdef double total_n = 0
    for i in range(nuclei):
        for j in range(i):
